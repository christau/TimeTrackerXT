...
public class TimeRange implements Comparable<TimeRange> {
     ...
     * day
     */
    public static long overlap( Calendar origDay, long start, long end ) {
        Calendar day = (Calendar)origDay.clone();
        for (int x : FIELDS) day.set(x, day.getMinimum(x));
        long ms_start = day.getTime().getTime();
        day.add(Calendar.DAY_OF_MONTH, 1);
        long ms_end = day.getTime().getTime();
        return overlap( ms_start, ms_end, start, end );
    }
        
    public static long overlap( long boundaryStart, long boundaryEnd, long timeStart, long timeEnd ) {        
        if (boundaryEnd < timeStart || timeEnd < boundaryStart) return 0;
        
        long off_start = boundaryStart > timeStart ? boundaryStart : timeStart;
        long off_end   = boundaryEnd < timeEnd ? boundaryEnd : timeEnd;
